...
if [ "$DATABRICKS_AUTH_TYPE" = "pat" ]; then
  ...
  if ! uvx databricks current-user me >/dev/null 2>&1; then
    ...
    echo "💡 Try running: uvx databricks auth login --host $DATABRICKS_HOST"
  ...
  if ! uvx databricks current-user me --profile "$DATABRICKS_CONFIG_PROFILE" >/dev/null 2>&1; then
    ...
    echo "💡 Try running: uvx databricks auth login --host <your-host> --profile $DATABRICKS_CONFIG_PROFILE"
    echo "💡 Or run ./setup.sh to reconfigure authentication"
    exit 1
  fi
...
" 2>/dev/null)
    ...
    APP_URL=$(uvx databricks apps get "$DATABRICKS_APP_NAME" --output json 2>/dev/null | python3 -c "
import json, sys
try:
    data = json.load(sys.stdin)
    print(data.get('url', 'URL not available'))
  ...
  if [ "$APP_EXISTS" -eq 0 ]; then
    ...
    if [ "$DATABRICKS_AUTH_TYPE" = "databricks-cli" ]; then
      if [ "$VERBOSE" = true ]; then
        ...
        uvx databricks apps create "$DATABRICKS_APP_NAME" --profile "$DATABRICKS_CONFIG_PROFILE" > /dev/null 2>&1
      fi
    else
      if [ "$VERBOSE" = true ]; then
        uvx databricks apps create "$DATABRICKS_APP_NAME"
      else
        uvx databricks apps create "$DATABRICKS_APP_NAME" > /dev/null 2>&1
      fi
    fi
...
if [ "$DATABRICKS_AUTH_TYPE" = "databricks-cli" ]; then
  if [ "$VERBOSE" = true ]; then
    ...
    uvx databricks apps deploy "$DATABRICKS_APP_NAME" --source-code-path "$DBA_SOURCE_CODE_PATH" --profile "$DATABRICKS_CONFIG_PROFILE"
  fi
else
  if [ "$VERBOSE" = true ]; then
    uvx databricks apps deploy "$DATABRICKS_APP_NAME" --source-code-path "$DBA_SOURCE_CODE_PATH" --debug
  else
    uvx databricks apps deploy "$DATABRICKS_APP_NAME" --source-code-path "$DBA_SOURCE_CODE_PATH"
  fi
fi
...
" 2>/dev/null)
  ...
  APP_URL=$(uvx databricks apps list --output json 2>/dev/null | python3 -c "
import json, sys
try:
    data = json.load(sys.stdin)
    if isinstance(data, list):
...
if [ -n "$APP_URL" ]; then
  ...
  echo ""
  echo "📊 Monitor deployment logs at (visit in browser):"
  echo "Check 'uvx databricks apps list' for the actual app URL, then add /logz"
fi
